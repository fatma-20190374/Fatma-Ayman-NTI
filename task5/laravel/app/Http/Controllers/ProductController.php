...
class ProductController extends Controller
{
    ...
    {
        $imageName = HasMedia::upload($request->file('image'),public_path('images'));
        $data = $request->except('_token','image');
        $data['image'] = $imageName;
        Product::create($data);
        return redirect()->route('dashboard.products.index')->with('success','Prodduct Created Successfully');
    }
    ...
    public function update(UpdateProductRequest $request, $id)
    {
        ...
        if($request->hasFile('image')){
            $imageName = HasMedia::upload($request->file('image'),public_path('images'));
            HasMedia::delete(public_path("images\\{$product->image}"));
            $data['image'] = $imageName;
        }
        $product->update($data);
