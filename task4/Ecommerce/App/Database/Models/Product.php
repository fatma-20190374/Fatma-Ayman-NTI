...
class Product extends Model  implements HasCrud  {
    ...
    public function newestProducts(): \mysqli_result
    {
    ...
    {
        $query = "SELECT products.* from products LEFT JOIN orders_products ON products.id = orders_products.product_id 
        GROUP BY products.id
        ORDER BY orders_products.quantity DESC
        LIMIT 4 ";
        $stmt =  $this->conn->prepare($query);
        $stmt->execute();
        return $stmt->get_result();
